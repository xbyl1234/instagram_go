...
func (r *RedisProcessor) redisConnection(redisInterface string, password string) (err error) {
	if r.isConnected == false {
		r.conn, err = RedisDialer("tcp", redisInterface, password)
		if err != nil {
			// handle error
			return err
		}
...
func Redis() Decorator {
	...
	Svc.AddInitializer(InitializeWith(func(backendConfig BackendConfig) error {
		...
		if redisErr := redisClient.redisConnection(config.RedisInterface, config.RedisPassword); redisErr != nil {
		...
		return ProcessWith(func(e *mail.Envelope, task SelectTask) (Result, error) {
			...
			if task == TaskSaveMail {
				...
				if len(e.Hashes) > 0 {
					...
					redisErr = redisClient.redisConnection(config.RedisInterface, config.RedisPassword)
					if redisErr != nil {
