...
func (c *redisClient) redisConnection(redisInterface string, password string) (err error) {
	if c.isConnected == false {
		c.conn, err = RedisDialer("tcp", redisInterface, password)
		if err != nil {
			// handle error
			return err
		}
	...
	return func(p Processor) Processor {
		return ProcessWith(func(e *mail.Envelope, task SelectTask) (Result, error) {
			if task == TaskSaveMail {
				...
				redisErr = redisClient.redisConnection(g.config.RedisInterface, g.config.RedisPassword)
				if redisErr == nil {
