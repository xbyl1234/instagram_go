...
func (this *VideoFeed) Next() (*VideosFeedResp, error) {
	...
	if err == nil {
		this.Inst.IncreaseSuccess(OperNameFeedVideo)
		this.LastMedias = ret
...
func (this *VideoFeed) NextPost() (*Media, error) {
	if this.media == nil || this.indexMedia >= len(this.media) {
		...
		if len(next.Items) == 0 {
			return nil, &common.MakeMoneyError{ErrStr: "not feed anymore"}
		}
		this.media = make([]*Media, len(next.Items))
		for index := range this.media {
			this.media[index] = next.Items[index].Media
		}
		this.indexMedia = 0
	}
